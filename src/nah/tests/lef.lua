...
function M.printResults()
    for i, result in ipairs(M.results) do
        ...
        M.print(i..'\t'..pass..' - '..result.message)
    end
    M.print('\nTotal number of tests:'..runCount)
end
...
local function assertEqual(message, expect, actual, failMessage)
    if expect == actual then
        ...
        fail("FAIL: " .. message .. " - " .. failMessage .. " Found " .. tostring(actual) .. " expected " .. tostring(expect))
    end
end
...
function T.testAddNilSystem(lef)
    lef.addSystem('testGroup', 'testSystem', 1, nil)
    assertEqual('checking total systems', 0, lef.systemCount())
end

function T.testAddNilNameSystem(lef)
    lef.addSystem('testGroup', nil, 1, function(e) end)
    assertEqual('checking total systems', 0, lef.systemCount())
end

function T.testAddNilGroupSystem(lef)
    lef.addSystem(nil, 'testSystem', 1, function(e) end)
    assertEqual('checking total systems', 0, lef.systemCount())
end

function T.testAddOneSystem(lef)
    lef.addSystem('testGroup', 'testSystem', 1, function(e) end)
...
function T.testAddTwoSystemsToOneGroup(lef)
    lef.addSystem('testGroup', 'testSystem', 1, function(e) end)
    lef.addSystem('testGroup', 'testSystem2', 1, function(e) end)
...
function T.testAddTwoSystemsToTwoGroups(lef)
    lef.addSystem('testGroup', 'testSystem', 1, function(e) end)
    lef.addSystem('testGroup2', 'testSystem', 1, function(e) end)
...
function T.testAddThreeSystemsToTwoGroups(lef)
    lef.addSystem('testGroup', 'testSystem', 1, function(e) end)
    lef.addSystem('testGroup2', 'testSystem', 1, function(e) end)
    lef.addSystem('testGroup2', 'testSystem2', 1, function(e) end)
...
function T.testUpdateOneSpecificSystem(lef)
    ...
    lef.addSystem('testGroup', 'testSystem1', 1, function(e) system1UpdateCount = system1UpdateCount + 1 end)
    lef.addSystem('testGroup', 'testSystem2', 1, function(e) system2UpdateCount = system2UpdateCount + 1 end)
    lef.addSystem('testGroup', 'testSystem3', 1, function(e) system3UpdateCount = system3UpdateCount + 1 end)
...
function T.testUpdateOneSpecificGroup(lef)
    ...
    lef.addSystem('testGroup', 'testSystem1', 1, function(e) system1UpdateCount = system1UpdateCount + 1 end)
    lef.addSystem('testGroup1', 'testSystem2', 1, function(e) system2UpdateCount = system2UpdateCount + 1 end)
    lef.addSystem('testGroup1', 'testSystem3', 1, function(e) system3UpdateCount = system3UpdateCount + 1 end)
...
function T.testRemovingOneUniqueSystem(lef)
    lef.addSystem('testGroup', 'testSystem1', 1, function(e) end)
    lef.addSystem('testGroup', 'testSystem2', 1, function(e) end)
...
function T.testRemovingUniqueSystemGroup(lef)
    lef.addSystem('testGroup1', 'testSystem1', 1, function(e) end)
    lef.addSystem('testGroup1', 'testSystem2', 1, function(e) end)
    lef.addSystem('testGroup2', 'testSystem3', 1, function(e) end)
...
function T.testOneEntityAddedToSystem(lef)
    ...
    lef.addSystem('testGroup', 'testSystem', 1, updateCountIncrementerSystem, 'countComponent')
...
function T.testEntityAddedToSystemAfterInitialAdd(lef)
    -- Setup
    local entityA = {}
    lef.addSystem('testGroup', 'testSystem', 1, function() end, 'C')
    lef.addEntityComponents(entityA)
    assertEqual('checking number of entities', 1, lef.entityCount())
...
function T.testSystemUpdateOrderWhenAddedInOrderSamePriority(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false

    lef.addSystem('testGroup', 'a', 1, function()
        aUpdated = true;
        assertEqual('Checking if a updated', true, aUpdated)
        assertEqual('Checking if b updated', false, bUpdated)
        assertEqual('Checking if c updated', false, cUpdated)
    end)
    lef.addSystem('testGroup', 'b', 1, function()
        bUpdated = true;
        assertEqual('Checking if a updated', true, aUpdated)
        assertEqual('Checking if b updated', true, bUpdated)
        assertEqual('Checking if c updated', false, cUpdated)
    end)
    lef.addSystem('testGroup', 'c', 1, function()
        cUpdated = true;
        assertEqual('Checking if a updated', true, aUpdated)
        assertEqual('Checking if b updated', true, bUpdated)
        assertEqual('Checking if c updated', true, cUpdated)
    end)

    lef.updateSystem()

    assertEqual('After update, checking if a updated', true, aUpdated)
    assertEqual('After update, checking if b updated', true, bUpdated)
    assertEqual('After update, checking if c updated', true, cUpdated)
end

function T.testSystemUpdateOrderWhenAddedInOrder(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false

    lef.addSystem('testGroup', 'a', 3, function()
        aUpdated = true;
        assertEqual('Checking if a updated', true, aUpdated)
        assertEqual('Checking if b updated', false, bUpdated)
        assertEqual('Checking if c updated', false, cUpdated)
    end)
    lef.addSystem('testGroup', 'b', 2, function()
        bUpdated = true;
        assertEqual('Checking if a updated', true, aUpdated)
        assertEqual('Checking if b updated', true, bUpdated)
        assertEqual('Checking if c updated', false, cUpdated)
...
function T.testSystemUpdateOrderWhenAddedInReverseOrder(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false
...
function T.testSystemUpdateOrderWhenAddedInNoOrder(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false
...
function T.testSystemUpdateOrderWhenAddedInOrderWithHighestPriorityRemovedBeforeUpdate(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false
    ...
    lef.addSystem('testGroup', 'a', 3, function()
        aUpdated = true;
        fail('System processor A should never be called')
    end)
    lef.addSystem('testGroup', 'b', 2, function()
        bUpdated = true;
        assertEqual('Checking if a updated', false, aUpdated)
        assertEqual('Checking if b updated', true, bUpdated)
        assertEqual('Checking if c updated', false, cUpdated)
    end)
    lef.addSystem('testGroup', 'c', 1, function()
    ...
    lef.removeSystem('testGroup', 'a')
    lef.updateSystem()

    assertEqual('After update, checking if a updated', false, aUpdated)
    assertEqual('After update, checking if b updated', true, bUpdated)
    assertEqual('After update, checking if c updated', true, cUpdated)
end

function T.testSystemUpdateOrderWhenAddedInOrderWithLowestPriorityRemovedBeforeUpdate(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false
    ...
    lef.addSystem('testGroup', 'c', 1, function()
        cUpdated = true
        fail('System processor c should never be called')
    end)

    lef.removeSystem('testGroup', 'c')
    lef.updateSystem()

    assertEqual('After update, checking if a updated', true, aUpdated)
    assertEqual('After update, checking if b updated', true, bUpdated)
    assertEqual('After update, checking if c updated', false, cUpdated)
end

function T.testSystemUpdateOrderWhenAddedInOrderWithMiddlePriorityRemovedBeforeUpdate(lef)
    local aUpdated = false
    local bUpdated = false
    local cUpdated = false
    ...
    lef.addSystem('testGroup', 'b', 2, function()
        bUpdated = true;
        fail('System processor b should never be called')
    end)
    lef.addSystem('testGroup', 'c', 1, function()
        cUpdated = true
        assertEqual('Checking if a updated', true, aUpdated)
    ...
    lef.removeSystem('testGroup', 'b')
    lef.updateSystem()

    assertEqual('After update, checking if a updated', true, aUpdated)
    assertEqual('After update, checking if b updated', false, bUpdated)
    assertEqual('After update, checking if c updated', true, cUpdated)
end
